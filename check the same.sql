...
SELECT
	*,
	ABS(updates_avg_var20 - case_avg_var20) AS difference
FROM
	(SELECT 
...
INNER JOIN
	(SELECT 
		AVG(var20)  AS case_avg_var20, 
		year        AS case_year
	FROM wide_case_when 
	GROUP BY year) AS b
ON a.updates_year = b.case_year
ORDER BY difference DESC

-- should be no examples of contradictions:
select top 50 a.id, a.year, a.var21, b.var21, c.value 
from wide_updates AS a 
inner join wide_case_when AS b on a.year = b.year and a.id = b.id 
